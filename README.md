...
网络协议学习笔记，整理自一次关于路由协议的系统对话。涵盖两大类内部网关协议（IGP）、边界网关协议（BGP），以及它们在真实网络中协同工作的综合实例。所有配置示例均以 Cisco IOS 语法给出。
...
先掌握一种 IGP（OSPF 最常见），理解"链路状态 + SPF + 区域分层"这套思路；再看 IS-IS，重点对比它与 OSPF 的差异（地址、分层、边界位置）；然后学 BGP，理解它"策略选路、AS 之间互联"的本质；最后看第 4 篇，把 IGP 与 BGP 串起来，理解真实网络中"IGP 管内部可达、BGP 管外部策略、靠递归查找衔接"的部署形态。
